...
source_run <- file.path(down_dir, "eems_analysis_ess200_extension3_2026-07-30")
input_run <- file.path(down_dir, "eems_analysis_ess200_2026-07-29")
run_dir <- file.path(down_dir, "eems_analysis_ess200_extension4_2026-08-04")
...
previous_paths <- file.path(
  source_run,
  "mcmc",
  paste0("chain", 1:4, "_fixed250_extension3")
)
restart_files <- c(
...
# The prior run retained 7,000 states and reached a minimum ESS of 129.1.
# Retaining 13,000 states should exceed ESS 200 under comparable mixing.
chain_plan <- data.frame(
  Chain = 1:4,
  Action = "continue",
  Seed = 20260804:20260807,
  Iterations = 14000000L,
  Burnin = 1000000L,
  Thin = 999L,
  Retained_states = 13000L,
  Previous_path = previous_paths
)
write.table(
  chain_plan,
  file.path(table_dir, "eems_extension4_chain_plan.tsv"),
  sep = "\t", row.names = FALSE, quote = FALSE
)

run_chain <- function(i) {
  chain <- chain_plan$Chain[i]
  mcmc_dir <- file.path(mcmc_root, sprintf("chain%d_fixed250_extension4", chain))
...
write.table(
  run_summary,
  file.path(table_dir, "eems_extension4_run_summary.tsv"),
  sep = "\t", row.names = FALSE, quote = FALSE
)
...
writeLines(
  ...
  file.path(run_dir, "EXTENSION4_COMPLETE.txt")
)
message("Fourth EEMS continuation completed: ", run_dir)
