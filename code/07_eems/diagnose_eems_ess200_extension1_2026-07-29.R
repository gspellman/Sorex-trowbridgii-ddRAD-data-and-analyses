...
run_dir <- "/Users/gspellman/Trowbridgii_analyses/Stacks analysis of ddRAD data/stacks_refmap_sorex_excl_MVZ216210_2026-02-26_clean/downstream_analysis_2026-02-23/eems_analysis_ess200_extension1_2026-07-29"
table_dir <- file.path(run_dir, "tables")
paths <- file.path(run_dir, "mcmc", paste0("chain", 1:4, "_fixed250_extension1"))

if (!all(file.exists(file.path(paths, "eemsrun.txt")))) stop("Continuation chains are incomplete")
...
write.table(ess, file.path(table_dir, "eems_extension1_parameter_ess.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)
write.table(rhat, file.path(table_dir, "eems_extension1_split_rhat.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)
write.table(halves, file.path(table_dir, "eems_extension1_half_means.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)
...
write.table(summary, file.path(table_dir, "eems_extension1_convergence_summary.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)
