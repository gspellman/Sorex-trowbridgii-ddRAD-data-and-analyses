...
source_run <- file.path(down_dir, "eems_analysis_ess200_extension2_2026-07-30")
input_run <- file.path(down_dir, "eems_analysis_ess200_2026-07-29")
run_dir <- file.path(down_dir, "eems_analysis_ess200_extension3_2026-07-30")
...
source_paths <- file.path(
  source_run,
  "mcmc",
  paste0("chain", 1:4, "_fixed250_extension2")
)
# Chain 3 is replaced from chain 2's high-posterior state. Its separate seed and
# extended burn-in allow it to move independently before retaining samples.
previous_paths <- source_paths
previous_paths[3] <- source_paths[2]
...
required <- c(
  ...
  unlist(lapply(previous_paths, function(p) file.path(p, restart_files))),
  runeems
)
...
chain_plan <- data.frame(
  Chain = 1:4,
  Action = c("continue", "continue", "replace_from_chain2", "continue"),
  Seed = 20260750:20260753,
  Iterations = c(8000000L, 8000000L, 10000000L, 8000000L),
  Burnin = c(1000000L, 1000000L, 3000000L, 1000000L),
  Thin = 999L,
  Retained_states = c(7000L, 7000L, 7000L, 7000L),
  Previous_path = previous_paths
)
write.table(
  chain_plan,
  file.path(table_dir, "eems_extension3_chain_plan.tsv"),
  sep = "\t", row.names = FALSE, quote = FALSE
)

run_chain <- function(i) {
  chain <- chain_plan$Chain[i]
  mcmc_dir <- file.path(mcmc_root, sprintf("chain%d_fixed250_extension3", chain))
...
write.table(
  run_summary,
  file.path(table_dir, "eems_extension3_run_summary.tsv"),
  sep = "\t", row.names = FALSE, quote = FALSE
)
...
writeLines(
  ...
  file.path(run_dir, "EXTENSION3_COMPLETE.txt")
)
message("Third EEMS continuation completed: ", run_dir)
