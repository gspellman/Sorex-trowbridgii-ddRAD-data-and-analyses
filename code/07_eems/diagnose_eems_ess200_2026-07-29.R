...
run_dir <- "/Users/gspellman/Trowbridgii_analyses/Stacks analysis of ddRAD data/stacks_refmap_sorex_excl_MVZ216210_2026-02-26_clean/downstream_analysis_2026-02-23/eems_analysis_ess200_2026-07-29"
table_dir <- file.path(run_dir, "tables")
paths <- file.path(run_dir, "mcmc", paste0("chain", 1:4, "_fixed250"))

if (!all(file.exists(file.path(paths, "eemsrun.txt")))) {
  stop("One or more long EEMS chains are incomplete")
}
...
split_rhat <- function(chains) {
  ...
  chain_means <- vapply(halves, mean, numeric(1))
  chain_vars <- vapply(halves, var, numeric(1))
...
for (filename in names(file_specs)) {
  ...
  per_chain <- lapply(paths, function(p) {
    ...
    if (ncol(x) != length(parameter_names)) stop("Unexpected columns in ", filename)
  ...
  for (parameter in parameter_names) {
    ...
    for (i in 1:4) {
      ...
      ess_rows[[length(ess_rows) + 1L]] <- data.frame(
        ...
        File = filename,
        Parameter = parameter,
        Retained_states = length(values),
        ESS = effective_size(values),
        Mean = mean(values),
        SD = sd(values)
      )
    }
...
write.table(
  ess, file.path(table_dir, "eems_ess200_parameter_ess.tsv"),
...
write.table(
  rhat, file.path(table_dir, "eems_ess200_split_rhat.tsv"),
  sep = "\t", row.names = FALSE, quote = FALSE
)

nonconstant_ess <- ess[is.finite(ess$ESS), ]
nonconstant_rhat <- rhat[is.finite(rhat$Split_Rhat), ]
summary <- data.frame(
  Minimum_per_chain_ESS = min(nonconstant_ess$ESS),
  Maximum_split_Rhat = max(nonconstant_rhat$Split_Rhat),
  ESS_target = 200,
  Rhat_target = 1.01,
  ESS_pass = min(nonconstant_ess$ESS) >= 200,
  Rhat_pass = max(nonconstant_rhat$Split_Rhat) <= 1.01
)
summary$Overall_pass <- summary$ESS_pass && summary$Rhat_pass
write.table(
  summary, file.path(table_dir, "eems_ess200_convergence_summary.tsv"),
  sep = "\t", row.names = FALSE, quote = FALSE
)
