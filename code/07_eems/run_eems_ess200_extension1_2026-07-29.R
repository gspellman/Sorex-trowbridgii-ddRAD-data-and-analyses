...
source_run <- file.path(down_dir, "eems_analysis_ess200_2026-07-29")
run_dir <- file.path(down_dir, "eems_analysis_ess200_extension1_2026-07-29")
data_prefix <- file.path(source_run, "data", "trow_eems")
grid_prefix <- file.path(source_run, "fixed_grid", "fixed_250_grid")
...
previous_paths <- file.path(source_run, "mcmc", paste0("chain", 1:4, "_fixed250"))
required <- c(
...
write.table(
  chain_plan,
  file.path(table_dir, "eems_extension1_chain_plan.tsv"),
  sep = "\t", row.names = FALSE, quote = FALSE
)

run_chain <- function(i) {
  chain <- chain_plan$Chain[i]
  mcmc_dir <- file.path(mcmc_root, sprintf("chain%d_fixed250_extension1", chain))
...
write.table(
  run_summary,
  file.path(table_dir, "eems_extension1_run_summary.tsv"),
  sep = "\t", row.names = FALSE, quote = FALSE
)
...
writeLines(
  ...
  file.path(run_dir, "EXTENSION1_COMPLETE.txt")
)
message("Tuned EEMS continuation completed: ", run_dir)
